...
### 开发任务
* 保证大模型对话的流式传输接口 "/chat/stream" 在使用transformers库时正常工作，需要保证大模型流式产生结果（目前需要等到大模型生成所有结果后才能返回），并通过stream接口流式传输。
* 基于mysql数据库开发登陆用api，包括注册和登陆，并在路由中注册相关接口（目前还没有数据库相关的保存和验证密码过程）
* 在当前的回答后面加上
...
* 简答问题
* 涉及数据库查询，需要设计专用提示词，使模型执行查询返回结果，查询结果通过文件呈现，
* 涉及小模型调用，需要设计另一套提示词
* 结果总结
